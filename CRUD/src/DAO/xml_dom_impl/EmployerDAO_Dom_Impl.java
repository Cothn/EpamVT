...
public class EmployerDAO_Dom_Impl implements EmployerDAO{
    ...
    public List<Employer> getAll() throws DaoException {
        ...
        try {
            ...
            NodeList employerNodes = root.getElementsByTagName("Employer");
            Employer employer = null;
            for(int i = 0; i < employerNodes.getLength();i++)
            {
                employer = new Employer();
                Element employerElement = (Element) employerNodes.item(i);
                employer.setId(Integer.parseInt(getChild(employerElement,"id").getTextContent().trim()));
                employer.setName(getChild(employerElement,"name").getTextContent().trim());
                employer.setSurname(getChild(employerElement,"surname").getTextContent().trim());
                employer.setPhone((getChild(employerElement,"phone").getTextContent().trim()));
                employer.setPosition((getChild(employerElement,"position").getTextContent().trim()));
